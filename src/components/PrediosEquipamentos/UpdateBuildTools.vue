...
export default {
  ...
  methods: {
    ...
    async updatePrediosEquipamento() {
      ...
      try {
        const response = await axios.put(
         `https://basck-durabil2.onrender.com/PrediosEquipamentos/${this.predios_equipamentos.id}`,
          data
        );
    ...
    try {
      const response = await axios.get(`https://basck-durabil2.onrender.com/PrediosEquipamentos/${this.predios_equipamentos.id}`);
