<template>
 ...
 <form>
  ...
  <v-text-field
    v-model="predios.numero_ocupantes"
    :counter="4"
    :error-messages="numero_ocupantes.errorMessage.value"
    label="Numero Ocupantes"
    @input="predios.numero_ocupantes = parseFloat(predios.numero_ocupantes)"
  ></v-text-field>

  <v-text-field
    v-model="predios.area"
    :error-messages="area.errorMessage.value"
    label="Área(m2)"
    @input="predios.area = parseFloat(predios.area)"
  ></v-text-field>
...
export default {
 ...
 methods: {
   ...
   async update() {
      ...
      try {
        const response = await axios.put(`http://localhost:3000/PrediosAmbiente/${this.predios.id}`, data)
