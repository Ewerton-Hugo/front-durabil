...
export default {
  ...
  methods: {
    ...
    try {
      const response = await axios.get(`https://basck-durabil2.onrender.com/PrediosAmbiente/${this.predios.id}`);
    ...
    async update() {
      ...
      try {
        const response = await axios.put(
          `https://basck-durabil2.onrender.com/PrediosAmbiente/${this.predios.id}`,
          data
        );
